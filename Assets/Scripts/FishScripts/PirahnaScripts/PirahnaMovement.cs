...
public class PirahnaMovement : MonoBehaviour
{
    Fish pirahna;
    FishSO pirahnaSO;
    ...
    private void Awake()
    {
        ...
        pirahnaState = GetComponent<FishState>();
    ...
    {
        switch (pirahnaState.GetCurrentState())
        {
    ...
    void MoveFish()
    {
        if (nextLocationTimer <= 0)
        {
            if (Vector2.Distance(transform.position, targetPosition) > 0.1f)
            {
                ...
                float easedMoveSpeed = Mathf.Lerp(pirahnaSO.moveSpeed, 0f, easedT); // Interpolate movement speed based on eased t
    ...
    void MoveFishToSpawn()
    {
        if (Vector2.Distance(transform.position, targetPosition) > 0.1f)
        {
            ...
            float spawnMoveSpeed = Mathf.Lerp(pirahnaSO.moveSpeed * 7, 0.5f, easedT); // Interpolate movement speed based on eased t
        ...
        else
        {
            ...
            if (enemiesInTank.Length == 0 || pirahnaSO.classes.Contains(FishClass.Resource))
            {
                pirahnaState.SetStateTo(FishState.State.Normal);
                PickRandomLocation();
            }
            else if (enemiesInTank != null && !pirahnaSO.classes.Contains(FishClass.Resource))
            {
                pirahnaState.SetStateTo(FishState.State.Combat);
            }
        }
    }
    ...
    void MoveFishToFood()
    {
        ...
        foreach (Fish f in preferredFood)
        {
            ...
            if (distance < pirahnaSO.foodDetectionRange)
            {
        ...
        float easedFishHungerMoveSpeed = Mathf.Lerp(pirahnaSO.moveSpeed, 0f, easedT); // Interpolate movement speed based on eased t
    ...
    void PickWaitTimer()
    {
        nextLocationTimer = Random.Range(pirahnaSO.minLocationPickTimer, pirahnaSO.maxLocationPickTimer);
    }
